...
<div class="wrap">
			...
			<form name="addwiki" id="addwiki" method="post" action="<?php $this->friendly_page_link( 'wiki' ); echo ( $current !== FALSE ? '&action=edit&id=' . $current[ 'wiki_id' ] : '' ); ?>">
				...
				<input name="referredby" type="hidden" id="referredby" value="<?php echo get_bloginfo('wpurl');?>/wp-admin/admin.php?page=wikipress" />

				<div id="poststuff" class="metabox-holder has-right-sidebar" style="position:absolute;right:15px;left:auto;width:330px;min-width:330px;">
		  ...
		  <div class="wikipressfg">
					...
					<div id="wikibody" style="width:97%;min-height:600px;background:#fff;margin:0 auto;">
						...
						<input 
							...
							class="wiki-input" 
							name="wiki_name" 
							value="<?php echo stripslashes($current[ 'wiki_name']); ?>" 
							style="padding:3px 5px; width:100%;border:1px solid #efefef; background:#fff; height:34px;color:#333;font-size:1.6em;font-weight:bold;margin-top:10px;"
						/><br />

						<textarea name="wiki_body" id="wiki_body" class="wiki-textarea" style="width:100%;border:1px solid #efefef;background:#fff;padding:5px;color:#333;font-size:1.1em;min-height:600px;"><?php echo ($current[ 'wiki_body']); ?></textarea>
						</div>
					</div>
